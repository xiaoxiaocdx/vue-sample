<template>
  <div class="welfare-wrapper" v-infinite-scroll="loadMore" infinite-scroll-disabled="busy" infinite-scroll-distance="10">
    ...
    <div class="welfare-center">
      <figure v-show="rightDate.length">0"v-for="data in rightData" @click="selectDetails(data.createAt)">
        <v-img :imgUrl="data.url"></v-img>
      </figure>
    </div>
...
<script>
  import {objectData} from '../../common/js/data';
  import vImg from '../lazyloading/lazing.vue';
  import vDetail from '../details/details.vue';
  export default{
    data(){
      return{
        leftData:[],
